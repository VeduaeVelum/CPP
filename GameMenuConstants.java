package main_classes;

/**
 * @Interface GameMenuConstants Interface keep game menu constants
 */

public interface GameMenuConstants {
  static final String FON_PATH = "fon.jpg";
  static final String NEW_GAME = "New Game";
  static final String SETTINGS = "Settings";
  static final String GAME_RULES = "Game rules";
  static final String REPLAYS = "Replays List";
  static final String EXIT = "Exit";
  static final String SOUND = "Sound: On";
  static final String BACK = "Back";
  static final String GAME_RULES_TITLE = "Game Rules";
  static final String SET_PARAMS_TITLE = "Setting configurations";
  static final String REPLAYS_TITLE = "Choose replay";
  static final String MENU_TITLE = "Game Menu";
  static final String CIRCLE_BUTTON_TEXT = "CHANGE\nBACKGROUND\n"
      + "CONFIGURATION";
  static final String CIRCLE_STYLE = "-fx-border-color:blue";

  static final int SCENE_SIZE = 700;
  static final int MENU_COORD_X = 150;
  static final int MENU_COORD_Y = 200;
  static final int CIRCLES_RADIUS = 20;
  static final int CIRCLES_DURATION = 5000;
  static final int CIRCLE_BUTTON_X = 175;
  static final int CIRCLE_BUTTON_Y = 250;
  static final int CIRCLE_BUTTON_RADIUS = 52;
  static final int CIRCLES_COUNT = 15;
  
  static final int STEP_BTWN_BUTTONS = 25;
  static final double BUTTON_ANIM_DURACTION = 0.5;
  static final double CIRCLE_BUTTON_OPACITY = 0.6;
  
}
